...
/******************************************************************************
       ...
       Return value: None
******************************************************************************/
#define LCD_Writ_Bus_Default(dat) LCD_Writ_Bus(dat,1)
void LCD_Writ_Bus(u8 dat, u8 flag) 
{
#if SPI0_CFG == 1
	OLED_CS_Clr();
...
/******************************************************************************
       ...
       Return value: None
******************************************************************************/
#define LCD_WR_DATA8_DEFAULT(dat) LCD_WR_DATA8(dat,1)
void LCD_WR_DATA8(u8 dat, u8 flag)
{
	OLED_DC_Set();//Write data
	LCD_Writ_Bus(dat,flag);
}
...
{
	OLED_DC_Set();//Write data
	LCD_Writ_Bus_Default(dat>>8);
	LCD_Writ_Bus_Default(dat);
}
...
void Lcd_Init(void)
{
	...
	LCD_WR_REG(0xB1);	// Set the frame frequency of the full colors normal mode
						...
						// fosc = 850kHz
	LCD_WR_DATA8_DEFAULT(0x05);	// RTNA
	LCD_WR_DATA8_DEFAULT(0x3A);	// FPA
	LCD_WR_DATA8_DEFAULT(0x3A);	// BPA

	LCD_WR_REG(0xB2);	// Set the frame frequency of the Idle mode
						// Frame rate=fosc/((RTNB x 2 + 40) x (LINE + FPB + BPB +2))
						// fosc = 850kHz
	LCD_WR_DATA8_DEFAULT(0x05);	// RTNB
	LCD_WR_DATA8_DEFAULT(0x3A);	// FPB
	LCD_WR_DATA8_DEFAULT(0x3A);	// BPB

	LCD_WR_REG(0xB3);	// Set the frame frequency of the Partial mode/ full colors
	LCD_WR_DATA8_DEFAULT(0x05);  
	LCD_WR_DATA8_DEFAULT(0x3A);
	LCD_WR_DATA8_DEFAULT(0x3A);
	LCD_WR_DATA8_DEFAULT(0x05);
	LCD_WR_DATA8_DEFAULT(0x3A);
	LCD_WR_DATA8_DEFAULT(0x3A);
	...
	LCD_WR_REG(0xB4);
	LCD_WR_DATA8_DEFAULT(0x03);

	LCD_WR_REG(0xC0);
	LCD_WR_DATA8_DEFAULT(0x62);
	LCD_WR_DATA8_DEFAULT(0x02);
	LCD_WR_DATA8_DEFAULT(0x04);

	LCD_WR_REG(0xC1);
	LCD_WR_DATA8_DEFAULT(0xC0);

	LCD_WR_REG(0xC2);
	LCD_WR_DATA8_DEFAULT(0x0D);
	LCD_WR_DATA8_DEFAULT(0x00);

	LCD_WR_REG(0xC3);
	LCD_WR_DATA8_DEFAULT(0x8D);
	LCD_WR_DATA8_DEFAULT(0x6A);   

	LCD_WR_REG(0xC4);
	LCD_WR_DATA8_DEFAULT(0x8D); 
	LCD_WR_DATA8_DEFAULT(0xEE); 

	LCD_WR_REG(0xC5);  /*VCOM*/
	LCD_WR_DATA8_DEFAULT(0x0E);    

	LCD_WR_REG(0xE0);
	LCD_WR_DATA8_DEFAULT(0x10);
	LCD_WR_DATA8_DEFAULT(0x0E);
	LCD_WR_DATA8_DEFAULT(0x02);
	LCD_WR_DATA8_DEFAULT(0x03);
	LCD_WR_DATA8_DEFAULT(0x0E);
	LCD_WR_DATA8_DEFAULT(0x07);
	LCD_WR_DATA8_DEFAULT(0x02);
	LCD_WR_DATA8_DEFAULT(0x07);
	LCD_WR_DATA8_DEFAULT(0x0A);
	LCD_WR_DATA8_DEFAULT(0x12);
	LCD_WR_DATA8_DEFAULT(0x27);
	LCD_WR_DATA8_DEFAULT(0x37);
	LCD_WR_DATA8_DEFAULT(0x00);
	LCD_WR_DATA8_DEFAULT(0x0D);
	LCD_WR_DATA8_DEFAULT(0x0E);
	LCD_WR_DATA8_DEFAULT(0x10);
	...
	LCD_WR_DATA8_DEFAULT(0x03);
	LCD_WR_DATA8_DEFAULT(0x03);
	LCD_WR_DATA8_DEFAULT(0x0F);
	LCD_WR_DATA8_DEFAULT(0x06);
	LCD_WR_DATA8_DEFAULT(0x02);
	LCD_WR_DATA8_DEFAULT(0x08);
	LCD_WR_DATA8_DEFAULT(0x0A);
	LCD_WR_DATA8_DEFAULT(0x13);
	LCD_WR_DATA8_DEFAULT(0x26);
	LCD_WR_DATA8_DEFAULT(0x36);
	LCD_WR_DATA8_DEFAULT(0x00);
	LCD_WR_DATA8_DEFAULT(0x0D);
	LCD_WR_DATA8_DEFAULT(0x0E);
	LCD_WR_DATA8_DEFAULT(0x10);
	...
	LCD_WR_DATA8_DEFAULT(0x05);	// 16-bit/pixel

	LCD_WR_REG(0x36);
	if(USE_HORIZONTAL==0)LCD_WR_DATA8_DEFAULT(0x08);
	else if(USE_HORIZONTAL==1)LCD_WR_DATA8_DEFAULT(0xC8);
	else if(USE_HORIZONTAL==2)LCD_WR_DATA8_DEFAULT(0x78);
	else LCD_WR_DATA8_DEFAULT(0xA8);
...
{  
	u8 i,j;
	u8 *temp,size1;
	if(size==16){temp=Hzk16;}//size
	if(size==32){temp=Hzk32;}
  LCD_Address_Set(x,y,x+size-1,y+size-1); //area
  size1=size*size/8;//byte
	temp+=index*size1;//start
	for(j=0;j<size1;j++)
	{
		for(i=0;i<8;i++)
		{
...
{          
	u16 i,j; 
	LCD_Address_Set(xsta,ysta,xend,yend);      //p
	for(i=ysta;i<=yend;i++)
	{													   	 	
		for(j=xsta;j<=xend;j++)LCD_WR_DATA(color);//p    
	} 					  	    
}
...
void LCD_DrawLine(u16 x1,u16 y1,u16 x2,u16 y2,u16 color)
{
	...
	uRow=x1;//p
	uCol=y1;
	if(delta_x>0)incx=1; //p
	else if (delta_x==0)incx=0;//p
	else {incx=-1;delta_x=-delta_x;}
	if(delta_y>0)incy=1;
	else if (delta_y==0)incy=0;//p
	else {incy=-1;delta_y=-delta_x;}
	if(delta_x>delta_y)distance=delta_x; //p
	else distance=delta_y;
	for(t=0;t<distance+1;t++)
	{
		LCD_DrawPoint(uRow,uCol,color);//p
		xerr+=delta_x;
		yerr+=delta_y;
		if(xerr>distance)
		{
...
void LCD_ShowLogo(void)
{
	...
	for(i=0;i<25600;i++)
	{
		LCD_WR_DATA8_DEFAULT(logo_bmp[i]);
	}			
}
...
void LCD_ShowBlack()
{
	...
	for(i=0;i<161*81*2;i++)
	{
		LCD_WR_DATA8_DEFAULT(BLACK);
	}			
}
